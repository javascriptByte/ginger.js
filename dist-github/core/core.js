...
const ginger = function ({ prefix = 'default', storageAdapter }) {
    const store = {}, subscriptions = {};
    let isUpdatePending = false;
    const _getWithoutMeta = (store) => {
    ...
    const triggerSubscriptions = (event) => {
        (subscriptions[event] || []).forEach((fn) => {
            fn();
        });
    };
    const getSnapshot = () => {
    ...
    return {
        ...
        unSubscribe: (event, handler) => {
            ...
            ;
            subscriptions[event] = subscriptions[event].filter((fn) => handler !== fn);
        },
